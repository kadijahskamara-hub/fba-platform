...
import type {
  CalcDocInput, CalcDocResult, CalcLineInput, CalcLineResult,
  ApprovalRequirement, ClientTotalsClaim, TaxCategory,
} from './types'
...
function pctOfMinor(minor: number, percent: number): number {
 ...
 */
export function deriveSellingMinor(
  costUnitMinor: number,
  method: 'markup' | 'margin',
  percent: number,
): { minor: number; invalid: boolean } {
  if (!Number.isFinite(percent)) return { minor: costUnitMinor, invalid: true }
  if (method === 'markup') {
    return { minor: roundHalfUp(costUnitMinor * (1 + percent / 100)), invalid: false }
  }
  // margin
  if (percent >= 100) return { minor: costUnitMinor, invalid: true }
  const raw = costUnitMinor / (1 - percent / 100)
  if (!Number.isFinite(raw)) return { minor: costUnitMinor, invalid: true }
  return { minor: roundHalfUp(raw), invalid: false }
}

/** Markup % from cost and selling (both minor units); null when cost is 0/unknown. */
export function markupPercent(costMinor: number | null, sellMinor: number): number | null {
...
export function marginPercent(costMinor: number | null, sellMinor: number): number | null {
  if (costMinor === null || sellMinor === 0) return null
  return ((sellMinor - costMinor) / sellMinor) * 100
}

// ── Tax ──────────────────────────────────────────────────────

function taxRateFor(category: TaxCategory, vatRegistered: boolean, rates: { standard: number; reduced: number }): number {
...
export function calculateLine(
  ...
  const costUnitMinor = costUnavailable ? null : toMinor(input.supplierCostUnit as number)
  ...
  if (sellUnitMinor < 0) { sellUnitMinor = 0; invalidPricing = true }
  ...
  return {
    ...
    quantity: qty,
    supplierCostUnit: costUnavailable ? null : fromMinor(costUnitMinor as number),
...
function procurementBasisMinor(input: CalcDocInput, lines: CalcLineResult[]): number {
...
function procurementFeeMinor(input: CalcDocInput, basisMinor: number): number {
  const cfg = input.procurementFee
  switch (cfg.type) {
    case 'none': return 0
    ...
    case 'tiered': {
      ...
      for (const t of tiers) {
        if (t.up_to === null || basisMajor <= t.up_to) return toMinor(t.value)
      }
...
export function evaluateApproval(input: CalcDocInput, lines: CalcLineResult[], feeOverridden: boolean): ApprovalRequirement {
  const t = input.thresholds
  const reasons: string[] = []
  let level: ApprovalRequirement['level'] = 'none'

  const raise = (to: 'commercial' | 'ultra' | 'blocked') => {
    const order = { none: 0, commercial: 1, ultra: 2, blocked: 3 } as const
...
export function calculateDocument(input: CalcDocInput): CalcDocResult {
  const lines = input.lines.map(l => calculateLine(l, input.vatRegistered, input.taxRates))

  const sumMinor = (xs: CalcLineResult[], f: (l: CalcLineResult) => number) =>
    xs.reduce((s, l) => s + toMinor(f(l)), 0)
  ...
  const feeMinor = feeOverridden ? toMinor(input.procurementFee.override as number) : calculatedFeeMinor

  // VAT by category (line level), plus fee VAT at the standard rate when registered.
  const vatByCategory: Partial<Record<TaxCategory, number>> = {}
  let vatMinor = 0
  for (const l of lines) {
 ...
 */
export function verifyClientTotals(claim: ClientTotalsClaim, computed: CalcDocResult): string[] {
  const mismatches: string[] = []
  const check = (name: string, claimed: number | undefined, actual: number) => {
...
export function resolveDepositPercent(
  ...
  rules: Array<{ min_order_value: number; deposit_percent: number }>,
  grossTotal: number,
): number {
  let result = defaultPercent
  const sorted = [...(rules ?? [])].sort((a, b) => a.min_order_value - b.min_order_value)
  for (const r of sorted) {
    if (grossTotal >= r.min_order_value) result = r.deposit_percent
  }
  return result
}
